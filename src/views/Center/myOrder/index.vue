        ...
        <table class="order-item" v-for="order in orderList" :key="order.id">
          ...
          <tbody>
            ...
            <tr v-for="(orderDetail, index) in order.orderDetailList" :key="orderDetail.id">
              ...
              <!-- 这里主要两点：1.后面四列跨的行数应该是购买的商品种类数，有几种商品就会有几行，后面四列就要跨几行 -->
              <!-- 2. 遍历时后四列只要一次就可以了，一个订单里有五种商品，那么后四列只展示一次，显示五种商品的总价就可以了 -->
...
export default {
  ...
  methods: {
    async getOrderList() {
      try {
        let result = await this.$http.reqOrderList(this.page, this.limit);
        if(result.code === 200) {
          console.log(result)
